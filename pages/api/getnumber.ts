...
export default async function handler(
  ...
  try {
    ...
    if (!country || typeof country !== 'string') {
      res.status(400).json({ message: 'Invalid or missing \'country\' parameter' });
      return;
    }
    ...
    try {
      const data = [{ email, phone_number: purchasedNumber.phoneNumber, full_number_details: purchasedNumber, region: country }];
